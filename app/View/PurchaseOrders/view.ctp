...
<script>
	function generateOrder(id)
	{
		...
		$('#loading').load(root+'/purchase_orders/generateOrderPdf/' + id)
...
if($value['PurchaseOrder']['order_number'] != '' || $value['PurchaseOrder']['order_number'] != null)
{
	echo $this->Html->tag('h3',$this->html->link('Ordenes de compra','/purchase_orders/').' &gt; Orden '.$value['PurchaseOrder']['order_number']);
}
else
{
	echo $this->Html->tag('h3',$this->html->link('Ordenes de compra','/purchase_orders/').' &gt; Solicitud '.$value['PurchaseOrder']['request_number']);
}
...
echo '<div class="acc_detail">
		...
		echo '<div class="info_03">
				<table>
					<tr>
						<td>'.$this->html->link('Ver detalle','/purchase_orders/view/'.$value['PurchaseOrder']['id'],array('class'=>'a_button')).'</td>
					</tr>';
			...
			if($this->RequestAction('/external_functions/verifiedAccess/'.$this->RequestAction('/external_functions/getIdDataSession')."/2/budgetOrder") == true && $value['PurchaseOrder']['upload_user'] == 0 )
			{
				echo '<tr>
			...
			if($this->RequestAction('/external_functions/verifiedAccess/'.$this->RequestAction('/external_functions/getIdDataSession')."/2/generatingBill") == true && $value['PurchaseOrder']['approved'] == 1)
			{
				echo '<tr>
			...
			if($this->RequestAction('/external_functions/verifiedAccess/'.$this->RequestAction('/external_functions/getIdDataSession')."/2/modifiedBudgetRequest") == true && $value['PurchaseOrder']['approved'] == 1 )
			{
				echo '<tr>
								...
								if($approve['active'] == 1)
								{
									...
									if(file_exists('files/purchase_orders/orders/'.$value['PurchaseOrder']['order_number'].'.pdf'))
									{
										...
										if($this->RequestAction('/external_functions/getIdDataSession') ==$value['User']['id'] || $this->RequestAction('/external_functions/verifiedAccess/'.$this->RequestAction('/external_functions/getIdDataSession')."/2/generateOrderPdf") == true)
										{
											if($ok == 0)
											{
												echo '<tr><td colspan="2">'.$this->Html->link('Descarga la orden de compra ang', '/files/purchase_orders/orders/'.$value['PurchaseOrder']['order_number'].'.pdf', array('class' => 'link_admin link_pdf', 'target' => '_blank')).'</td></tr>';
											}
										}
