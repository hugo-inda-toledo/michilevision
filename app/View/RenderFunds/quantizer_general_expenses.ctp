<?php
	...
	{
	?>
		<h3><?php echo $this->html->link('Fondos por rendir','/render_funds/'); ?> &gt; <?php echo $this->html->link('Estadisticas e Informes','/render_funds/statsMenu'); ?> &gt; Cuantificador de gastos (General)</h3>
	...
	else
	{
		if($data != false)
		{
			...
			<div class="ViewTitleOptions">
					<?php
							echo $this->Html->tag('strong', 'Resultado para la busqueda general de fondos desde el '.$data['Details']['from_date'].' hasta el '.$data['Details']['to_date'], array('class' => 'link_admin link_report'));
					?>
			</div>
			...
			<table>
				...
				<tr>
					<td>Euro</td>
					<td><?php echo 'â‚¬ $'.number_format($data['Details']['euro_total'], 0, null, '.'); ?></td>
					<td><?php echo 'â‚¬ $'.number_format($data['Details']['euro_total_render'], 0, null, '.'); ?></td>
					<td><font color="red"><?php echo 'â‚¬ $'.number_format($data['Details']['euro_total'] - $data['Details']['euro_total_render'], 0, null, '.'); ?></font></td>
				</tr>
				<tr>
					<td>UF</td>
									...
									if($data['ApprovedFunds']['RenderFunds']  != false)
									{
										...
										foreach($data['ApprovedFunds']['RenderFunds'] as $renderFund)
										{
											...
											echo '<div class="header" id="header_'.$z.'">
														<div class="fr_date tip_tip_default link_admin link_calendar" title="Fecha de emisiÃ³n de la solicitud">'.$fecha_fondo.'</div>
														<div class="fr_time tip_tip_default link_admin link_clock" title="Hora de emisiÃ³n de la solicitud">'.substr($renderFund['RenderFund']['created'], 11, 5).'</div>
												...
												if($renderFund['RenderFund']['state_id'] == 2){
													...
													echo '<div class="fr_status fr_approved tip_tip_default" id="status_'.$z.'" title="Fondo NÂ° '.$renderFund['RenderFund']['fund_number'].' | Status : RENDIDO">'.$renderFund['State']['state'].' (R)</div>';
													}
											
													else{
														if($renderFund['RenderFund']['deliver'] == 1){
															echo '<div class="fr_status fr_approved tip_tip_default" id="status_'.$z.'" title="Fondo NÂ° '.$renderFund['RenderFund']['fund_number'].' | Status : ENTREGADO">'.$renderFund['State']['state'].' (E)</div>';
														}
														else{
			...
			<div class="ViewTitleOptions">
				...
				<?php
					...
					echo $this->html->link('Descargar en PDF','/render_funds/quantizer_expenses_general_report/'.$forDownload['RenderFund']['from_date'].'/'.$forDownload['RenderFund']['to_date'], 
					array('class' => 'tip_tip_default link_admin link_pdf', 'title' => 'Descargar en version PDF'));
					
					echo "<br /><br />";
					
					echo $this->html->link('Realizar otra busqueda','/render_funds/quantizerGeneralExpenses/',
