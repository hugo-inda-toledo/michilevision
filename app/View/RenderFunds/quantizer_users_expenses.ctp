<?php
	...
	{
	?>
		<h3><?php echo $this->html->link('Fondos por rendir','/render_funds/'); ?> &gt; <?php echo $this->html->link('Estadisticas e Informes','/render_funds/statsMenu'); ?> &gt; Cuantificador de gastos (Usuarios)</h3>
		...
		<script>
			...
			<?php
				...
				echo $this->form->input('user', array('label' => 'Solicitante', 'type' => 'select', 'options' => $users, 'default' => 0));
	...
	else
	{
		if($data != false)
		{
			...
			<div class="ViewTitleOptions">
					<?php
							echo $this->Html->tag('strong', 'Resultado para la busqueda de fondos del usuario '.$data['Details']['user'], array('class' => 'link_admin link_report'));
					?>
			</div>
			...
			<div class="ViewTitleOptions">
				...
				<?php
					...
					echo $this->html->link('Descargar en PDF','/render_funds/quantizer_expenses_user_report/'.$forDownload['RenderFund']['user'].'/'.$forDownload['RenderFund']['from_date'].'/'.$forDownload['RenderFund']['to_date'], 
					array('class' => 'tip_tip_default link_admin link_pdf', 'title' => 'Descargar en version PDF'));
					
					echo "<br /><br />";
					
					echo $this->html->link('Realizar otra busqueda','/render_funds/quantizerUsersExpenses/',
